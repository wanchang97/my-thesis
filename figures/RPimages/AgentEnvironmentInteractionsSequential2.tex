...
\begin{document}
	\begin{tikzpicture}[node distance=1.5cm]
		\node (Env) [env] {Environment(Bridge In Operation)};
		\node (Agent) [agent, right of= Env,xshift = 85mm] {Agent(Bridge Manager)};
		...
		\draw[draw=white, fill opacity = 0.5,fill = black!10] (-2,-3) rectangle ++(16,6);
		...
		% add time axis
		\draw [purple,->latex] (-2,0.5)-- ++(7/3*2,-8/3)  node[below,xshift = 5mm] {Time } ;
		\draw (-2+.2,-0.3) node[purple] {t};
		\draw (-1.2,-0.8) node[purple] {t+1};
		\draw (2.5,-2.5) node[purple] {T};
		
		% add time axis
		\draw [purple,->latex] (7.2,0.5)-- ++(7/3*2,-8/3)  node[below,xshift = 5mm] {Time } ;
		\draw (-2+9.2+0.2,-0.3) node[purple] {t};
		\draw (-1+9.2+0.2,-0.8) node[purple] {t+1};
		\draw (2.5+9.2,-2.5) node[purple] {T};
	\end{tikzpicture}		
\end{document}