...
\begin{document}
	...
	\begin{tikzpicture}[
		...
		\node[above = of s1,align=center,xshift= 10mm,yshift = -4mm] (S){ $\mathds{S} = \{s_1,s_2\}$, $\mathbf{\Omega} = \{o_1,o_2\}$, $\mathbf{T} = \mathbf{T}(s)$, $\mathds{O} = \mathds{O}(o|s')$};
		
		\node[above = of S,align=center,xshift= 1mm,yshift = -4mm]  {Hidden Markov Model: $4-$tuple $(\mathds{S},\mathbf{\Omega},\mathbf{T},\mathds{O})$};
	...
	%	\node[below= of TP,align=center,xshift= 0mm,yshift = 5mm] (next) {$\pi(s'|s)$: Next state $s'$ is determined \\ only by the current state $s$};
		\node[below= of TP,align=center,xshift= 0mm,yshift = 5mm] (next) {Sensor Model: $\mathds{O} = \mathds{O}(o|s)$ \\We are unsure which state we are in \\e.g. $p(o_1|s_1) = 0.75$, $p(o_2|s_2) = 0.75$};	
	\end{tikzpicture}
	...
	\begin{tikzpicture}[
		...
		% denote
		\node[above = of s1,align=center,xshift= 8mm,yshift = -4mm](S)  {$\mathds{S} = \{s_1,s_2\}$,$\mathbf{\Omega} = \{o_1,o_2\}$,$\mathds{A} = \{a_1,a_2\}$, $\mathbf{T} = \mathbf{T}(s,a)$, $\mathbf{R} = \mathbf{R}(s,a)$,$\mathds{O} = \mathds{O}(o|s')$ };
		\node[above = of S,align=center,xshift= 1mm,yshift = -4mm]  {Partially Observalbe Markov Decision Process: 6-tuple $(\mathds{S},\mathbf{\Omega},\mathds{A},\mathbf{T},\mathbf{R},\mathds{O})$};
		\node[below = of s1,align=center,xshift= 8mm,yshift = 4mm] (TP)  { Transition Model: $\pi(s'|s,a)$, for discrete states are transition matrix \\$
			\mathbf{T}  = [\pi(s'|s,a)] =
			\begin{array}{ccc}
				&  &\begin{array}{cc} s_1 & s_2 \end{array} \\
	...
	%	\node[below= of TP,align=center,xshift= 0mm,yshift = 5mm] (next) { $\pi(s'|s,a)$: Next state $s'$ is determined by \\ current state $s$ and current action $a$};
		\node[below = of TP,align=center,xshift= 0mm,yshift = 4mm] (reward)  { Reward Model $r(s,a)$ or $r(s,a,s')$ \\$
			...
			\end{array}
			$};
		\node[below= of reward,align=center,xshift= 0mm,yshift = 5mm] (next) {Sensor model $\mathds{O}(o|s')$ or $\mathds{O}(o|s',a)$ \\ We are unsure which state we are in\\e.g. $p(o_1|s_1) = 0.75$, $p(o_2|s_2) = 0.75$};	
	\end{tikzpicture}
\end{document}
