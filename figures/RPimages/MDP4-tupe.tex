...
\begin{document}
	...
	\begin{tikzpicture}[
		...
		\node[below = of S,align=center,xshift= 0mm,yshift = 4mm] (TP)  { Transition Model: $\pi(s'|s,a)$, for discrete states we use transition matrix \\$
			\mathbf{T}  = [\pi(s'|s,a)] =
			\begin{array}{ccc}
				& &\begin{array}{cc} s_1 & s_2 \end{array} \\
				\begin{array}{c}
		...
		\node[below= of reward,align=center,xshift= 0mm,yshift = 5mm] (next) {Sensor model: Conditional Probability of Observation
			...
			\begin{array}{ccc}
				...
				\end{array} \right]
			\end{array}
			$ %\\ We are unsure which state we are in\\e.g. $p(o_1|s_1) = 0.75$, $p(o_2|s_2) = 0.75$
		};	
	\end{tikzpicture}
