...
\begin{document}
	...
	%	\foreach \y in {-1.0, -0.7,-0.4,0.4, 0.7,1.0}
		\draw[fill=blue!30,line width=0.5mm] (3.45,0.25) rectangle (3.75,-0.25) ;
		\node (output) at (3.6, 0) { };
	...
	%	\draw [thick, decorate,decoration={brace,amplitude=10pt},yshift=2pt] (4,1) -- (4,-1) node [black,midway,xshift = 18pt,yshift=3pt] {$|\mathds{A}|$};
		...
		\node [fill = red!30!,below =of CNN,yshift = -5mm,xshift = 11mm](NN) {Deep NN $V(s_t;\thetaV)$};
		\node [fill = green!30,left=of NN] (st){$s_t$};
		
		% Q-function label
		\node [fill = blue!30,right=of NN] (Q) {$\tilde{V}(s_t)$};
